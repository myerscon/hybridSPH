...
pysph_sim.initialize_pysph_shocktube(dx=dx,xmin=0.0,xmax=1.0,ymin=0.0,ymax=0.05,gamma=1.4,DoDomain=True,
                                            mirror_x=True,mirror_y=True,adaptive=False,cfl=0.5,pfreq=10000,
                                            tf=0.012,dt=4e-5,scheme='adke',scheme_params=params[0])
...
if (fixed):
    ...
    while (pysph_sim.pysph_sim.solver.t<pysph_sim.pysph_sim.solver.tf):
        ...
        if ((count)%10==0):
            ...
            pysph_sim.pysph_sim.solver.dt=pysph_sim.pysph_sim.cfl*pysph_sim.pysph_sim.particles[0].dt_cfl.min()
        if (count > max_steps):
            break
    end = time.time()
